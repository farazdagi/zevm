...
pub inline fn log0(stack: *Stack) !void {
    _ = stack;
...
/// Note: This operation requires access to memory and log state.
/// It will be handled specially in the interpreter's execute() function.
pub inline fn log1(stack: *Stack) !void {
    _ = stack;
    return error.UnimplementedOpcode;
}
...
/// Note: This operation requires access to memory and log state.
/// It will be handled specially in the interpreter's execute() function.
pub inline fn log2(stack: *Stack) !void {
    _ = stack;
    return error.UnimplementedOpcode;
}
...
/// Note: This operation requires access to memory and log state.
/// It will be handled specially in the interpreter's execute() function.
pub inline fn log3(stack: *Stack) !void {
    _ = stack;
    return error.UnimplementedOpcode;
}
...
/// Note: This operation requires access to memory and log state.
/// It will be handled specially in the interpreter's execute() function.
pub inline fn log4(stack: *Stack) !void {
    _ = stack;
    return error.UnimplementedOpcode;
}
...
test "logging: all operations unimplemented" {
    ...
    try expectError(error.UnimplementedOpcode, log0(&stack));

    // LOG1
    try stack.push(U256.fromU64(32)); // length
    try stack.push(U256.ZERO); // offset
    try stack.push(U256.ZERO); // topic1
    try expectError(error.UnimplementedOpcode, log1(&stack));

    // LOG2
    for (0..4) |_| try stack.push(U256.ZERO);
    try expectError(error.UnimplementedOpcode, log2(&stack));

    // LOG3
    for (0..5) |_| try stack.push(U256.ZERO);
    try expectError(error.UnimplementedOpcode, log3(&stack));

    // LOG4
    for (0..6) |_| try stack.push(U256.ZERO);
    try expectError(error.UnimplementedOpcode, log4(&stack));
}
