...
test "MOD" {
    const test_cases = [_]TestCase{
        ...
        .{
            .name = "modulo by zero returns 0 (10 % 0)",
            .bytecode = &[_]u8{
                0x60, 0x00, // PUSH1 0
                0x60, 0x0A, // PUSH1 10
                0x06, // MOD
    ...
    const test_cases = [_]TestCase{
        .{
            ...
            .bytecode = &[_]u8{
                ...
                0x0B, // SIGNEXTEND -> signextend(0x7F, 0)
                0x00, // STOP
            },
            // Should remain 0x7F
        ...
        .{
            ...
            .bytecode = &[_]u8{
                ...
                0x0B, // SIGNEXTEND -> signextend(0xFF, 0)
                0x00, // STOP
            },
        ...
        .{
            ...
            .bytecode = &[_]u8{
                0x60, 0x01, // PUSH1 1
                0x61, 0x7F, 0xFF, // PUSH2
                0x0B, // SIGNEXTEND -> signextend(0x7FFF, 1)
                0x00, // STOP
            },
            // Should remain 0x7FFF
            .expected_stack = &[_]U256{U256.fromU64(0x7FFF)},
            .expected_gas = 11, // PUSH1(3) + PUSH2(3) + SIGNEXTEND(5) + STOP(0)
        },
        .{
            .name = "byte 1 negative (0x8FFF)",
            .bytecode = &[_]u8{
                0x60, 0x01, // PUSH1 1
                0x61, 0x8F, 0xFF, // PUSH2
                0x0B, // SIGNEXTEND -> signextend(0x8FFF, 1)
                0x00, // STOP
            },
        ...
        .{
            ...
            .bytecode = &[_]u8{
                ...
                0x0B, // SIGNEXTEND -> signextend(0x1234567890, 31)
                0x00, // STOP
            },
            // Should remain unchanged
            .expected_stack = &[_]U256{U256.fromU64(0x1234567890)},
            .expected_gas = 11, // PUSH1(3) + PUSH5(3) + SIGNEXTEND(5) + STOP(0)
        },
        .{
        ...
        .{
            ...
            .bytecode = &[_]u8{
                0x60, 0x00, // PUSH1 0
                0x63, 0xFF, 0xFF, 0xFF, 0x7F, // PUSH4 0xFFFFFF7F (value)
                0x0B, // SIGNEXTEND -> signextend(0xFFFFFF7F, 0)
                0x00, // STOP
            },
            // Bit 7 of byte 0 is 0 (0x7F), so should clear all higher bits
            .expected_stack = &[_]U256{U256.fromU64(0x7F)},
            .expected_gas = 11, // PUSH1(3) + PUSH4(3) + SIGNEXTEND(5) + STOP(0)
        },
    };
    try runOpcodeTests(std.testing.allocator, &test_cases);
}
